...
namespace DynamicRouting.Kentico.Wildcards.Helpers
{
    public static class WildcardRoutingInitialiser
    {
        ...
        public static void OverrideRequestPageEvent(object sender, RequestRoutingEventArgs args)
        {
            ...
            var urlMatchup = GetUrlMatch(relativeUrl, args.Page?.DocumentCulture ?? "");
            if (urlMatchup != null)
            {
        ...
        {
            return CacheHelper.Cache<DataTable>(csdt => ConnectionHelper.ExecuteQuery("DynamicRouting.WildcardUrl.GetPossibleUrls", new QueryDataParameters() { { "@RelativeUrl", relativeUrl }, { "@CultureCode", cultureCode } }).Tables[0], new CacheSettings(1440, "possibleUrlPatterns" + GetUrlCacheName(relativeUrl)));
        }
        ...
        public static void OverrideGetPageEvent(object sender, GetPageEventArgs args)
        {
            ...
            HttpContext.Current.Items.Add("CurrentRelUrl", args.RelativeUrl);
